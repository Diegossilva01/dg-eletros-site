...
let PLANILHA_EXECUCAO_;
function planilha_(){return PLANILHA_EXECUCAO_||(PLANILHA_EXECUCAO_=SpreadsheetApp.openById(PLANILHA_ID));}
function cacheGet_(chave){try{return CacheService.getScriptCache().get(chave)}catch(_){return null}}
function cachePut_(chave,valor,segundos){try{CacheService.getScriptCache().put(chave,valor,segundos)}catch(_){/* cache opcional */}}
function cacheRemove_(chave){try{CacheService.getScriptCache().remove(chave)}catch(_){/* cache opcional */}}
...
function localizarCabecalho_(sh){
  if(!sh)return 1;
  if(sh.getName()!==CFG.ABAS.PRODUTOS)return 1;
  const chave='cabecalho-produtos-v3',salvo=Number(cacheGet_(chave));if(salvo>=1&&salvo<=50)return salvo;
  ...
  for(let i=0;i<vals.length;i++){
    ...
    if(norm.includes('produto')&&norm.includes('categoria')&&(norm.includes('preço')||norm.includes('preco'))){cachePut_(chave,String(i+1),3600);return i+1;}
  }
  return 1;
}
...
function responderPublico_(e){
  ...
  const chave='catalogo-publico-v2';
  const pronto=cacheGet_(chave);if(pronto!==null)return responder_(e,{ok:true,html:pronto});
  ...
  const conteudo=html||'<div class="sem-produtos">Nenhum produto cadastrado.</div>';
  if(conteudo.length<45000)cachePut_(chave,conteudo,45);
  return responder_(e,{ok:true,html:conteudo});
}
...
function auth_(token){if(!token)throw new Error('Sessão expirada.');const chave='sessao-'+hash_(token).slice(0,45),salva=cacheGet_(chave);if(salva){const u=JSON.parse(salva);if(u.validade>Date.now())return u.usuario;cacheRemove_(chave)}const sh=planilha_().getSheetByName(CFG.ABAS.USUARIOS),u=objetos_(sh).find(x=>String(x.Token||'')===String(token)&&x.Status==='Ativo');const validade=u?new Date(u['Validade token']).getTime():0;if(!u||validade<Date.now())throw new Error('Sessão expirada.');cachePut_(chave,JSON.stringify({usuario:u,validade}),Math.max(1,Math.min(60,Math.floor((validade-Date.now())/1000))));return u}function admin_(u){if(u.Perfil!=='Administrador')throw new Error('Acesso permitido somente ao administrador.')}function gerenciaProdutos_(u){if(!['Administrador','Vendedora','Vendedor'].includes(String(u.Perfil||'')))throw new Error('Você não tem permissão para gerenciar produtos.')}function publicUser_(u){return{id:u.ID,nome:u.Nome,usuario:u['Usuário'],perfil:u.Perfil}}
...
function excluirProduto_(u,d){gerenciaProdutos_(u);const sh=planilha_().getSheetByName(CFG.ABAS.PRODUTOS),todos=objetos_(sh);const id=String(d.id||'').trim();let r=id?todos.find(x=>String(x.ID||'').trim()===id):null;if(!r&&Number(d.linha)>0)r=todos.find(x=>Number(x._linha)===Number(d.linha)&&(!id||!x.ID||String(x.ID).trim()===id));if(!r)throw new Error('Produto não encontrado. Atualize a página e tente novamente.');sh.deleteRow(r._linha);cacheRemove_('catalogo-publico-v2');log_(u,'Produto',id||String(d.linha),d.motivo);return{ok:true}}
...
function listarVendas_(){
  const salvo=cacheGet_('vendas-v3');if(salvo)return JSON.parse(salvo);
  const sh=planilha_().getSheetByName(CFG.ABAS.VENDAS);
  const registros=objetos_(sh);
  const custos={};
  if(registros.some(x=>x['ID Produto']&&((x['Custo unitário']===''||x['Custo unitário']===undefined)&&(x['Custo total']===''||x['Custo total']===undefined)))){listarProdutos_().produtos.forEach(p=>{custos[String(p.id||'')]=num_(p.precoCusto)})}
  const vendas=registros.reverse().map(x=>{
  ...
  const resultado={ok:true,vendas},compacto=JSON.stringify(resultado);if(compacto.length<45000)cachePut_('vendas-v3',compacto,45);return resultado;
}
...
function listarComissoes_(u){
  ...
  const sh=planilha_().getSheetByName(CFG.ABAS.COMISSOES);
...
function abaHorasExtras_(){
  return planilha_().getSheetByName(CFG.ABAS.HORAS_EXTRAS)||criarAba_(planilha_(),CFG.ABAS.HORAS_EXTRAS,['ID','Data','ID Funcionário','Funcionário','Minutos','Motivo','Salário base','Horas semanais','Divisor mensal','Valor hora','Valor total','Status','Data de cadastro']);
}
...
function atualizarLinha_(sh,linha,obj){
  ...
  if(sh.getName()===CFG.ABAS.PRODUTOS)cacheRemove_('catalogo-publico-v2');
  if(sh.getName()===CFG.ABAS.VENDAS)cacheRemove_('vendas-v3');
}
...
function corrigirEstruturaProdutos_(){
  ...
  sh.setFrozenRows(info.linha);
  cacheRemove_('cabecalho-produtos-v3');
...
// Ponte interna do painel Google: reaproveita as mesmas permissões e ações do doPost.
function apiInterna(d){
  const saida=doPost({postData:{contents:JSON.stringify(d||{})}});
