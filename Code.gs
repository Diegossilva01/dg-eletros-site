const CFG={ABAS:{PRODUTOS:'Produtos',VENDAS:'Vendas',USUARIOS:'Usuarios',LOG:'LogExclusoes',COMISSOES:'Pagamentos Comissoes'},TOKEN_HORAS:720,PASTA:'DG Eletros - Fotos dos Produtos'};
...
function configurarSistema(){const ss=planilha_();criarAba_(ss,CFG.ABAS.PRODUTOS,['Produto','Categoria','Preço','Preço de custo','Classificação','Descrição','Link da Imagem','Promoção','WhatsApp','ID','Marca','Modelo','Quantidade','Status','Fotos','Data de cadastro']);criarAba_(ss,CFG.ABAS.VENDAS,['ID Venda','Data da venda','Data de cadastro','Cliente','CPF/CNPJ','Telefone','ID Produto','Produto','Quantidade','Valor da venda','Custo unitário','Custo total','Lucro','Forma de pagamento','Garantia dias','Observações','Vendedora','Origem do produto','Marca','Modelo','Categoria','Entrada','Saldo a receber','Parcelas JSON','Status recebimento','Recebimentos JSON']);criarAba_(ss,CFG.ABAS.USUARIOS,['ID','Nome','Usuário','Senha Hash','Perfil','Status','Token','Validade token','Data de cadastro']);criarAba_(ss,CFG.ABAS.LOG,['Data','Tipo','ID','Motivo','Usuário']);criarAba_(ss,CFG.ABAS.COMISSOES,['ID','Data do pagamento','Vendedora','Valor pago','Referência','Registrado por']);migrarProdutosExistentes_(ss.getSheetByName(CFG.ABAS.PRODUTOS));const sh=ss.getSheetByName(CFG.ABAS.USUARIOS);if(sh.getLastRow()<2)sh.appendRow([uid_('USR'),'Administrador','admin',hash_('123456'),'Administrador','Ativo','','',new Date()]);return 'Sistema configurado. Login inicial: admin / 123456';}
...
function cadastrarVenda_(u,d){
  ...
  const vs=criarAba_(ss,CFG.ABAS.VENDAS,['ID Venda','Data da venda','Data de cadastro','Cliente','CPF/CNPJ','Telefone','ID Produto','Produto','Quantidade','Valor da venda','Custo unitário','Custo total','Lucro','Forma de pagamento','Garantia dias','Observações','Vendedora','Origem do produto','Marca','Modelo','Categoria','Entrada','Saldo a receber','Parcelas JSON','Status recebimento','Recebimentos JSON']);
...
function listarVendas_(){
  ...
  const vendas=objetos_(sh).reverse().map(x=>{
    ...
    let parcelas=[];try{parcelas=JSON.parse(x['Parcelas JSON']||'[]')}catch(e){}let recebimentos=[];try{recebimentos=JSON.parse(x['Recebimentos JSON']||'[]')}catch(e){}const totalParcelas=parcelas.length,parcelasPagas=parcelas.filter(p=>String(p.status||'Pendente')==='Pago').length;return{id:x['ID Venda'],data:fmtData_(x['Data da venda']),cliente:x.Cliente,documento:x['CPF/CNPJ'],telefone:x.Telefone,idProduto,produto:x.Produto,quantidade,valor,custoUnitario,custoTotal,lucro,pagamento:x['Forma de pagamento'],garantia:num_(x['Garantia dias']),observacoes:x['Observações'],vendedora:x.Vendedora,origem:x['Origem do produto']||((x['ID Produto'])?'Estoque':'Venda avulsa'),entrada:num_(x.Entrada),saldo:num_(x['Saldo a receber']),parcelas,totalParcelas,parcelasPagas,statusRecebimento:x['Status recebimento']||'',recebimentos};
  });
...
function listarComissoes_(u){
  ...
  const itens=vendas.map(v=>{
    ...
    return {...v,recebido,pendente,comissaoGerada:Number((valor*percentual).toFixed(2)),comissaoPendente:0,comissaoTotal:Number((valor*percentual).toFixed(2))};
  });
  ...
  itens.sort((a,b)=>String(a.data||'').localeCompare(String(b.data||''))).forEach(v=>{
    const chave=String(v.vendedora||'Sem vendedor').trim().toLowerCase(),disponivel=num_(v.comissaoGerada);
  ...
  const itensAbertos=itens.filter(v=>num_(v.comissaoLiberada)>0.009);
  const totalGerado=itens.reduce((s,v)=>s+num_(v.comissaoGerada),0),totalPendente=0,totalPago=pagamentos.reduce((s,p)=>s+num_(p.valor),0),saldoPagar=Math.max(0,totalGerado-totalPago);
  ...
  const vendedores={};
  itens.forEach(v=>{const nome=v.vendedora||'Sem vendedor';if(!vendedores[nome])vendedores[nome]={vendedora:nome,gerada:0,liberada:0,pendente:0,paga:0};vendedores[nome].gerada+=num_(v.comissaoGerada);vendedores[nome].liberada+=num_(v.comissaoLiberada)});
  ...
  return{ok:true,percentual:2,totalHoje:liberadoHoje,totalMes:liberadoMes,totalLiberado:Number(saldoPagar.toFixed(2)),totalGerado:Number(totalGerado.toFixed(2)),totalPendente,totalPago:Number(totalPago.toFixed(2)),saldoPagar:Number(saldoPagar.toFixed(2)),vendasMes:itens.filter(v=>isoData_(v.data).slice(0,7)===mes).length,itens:itensAbertos,pagamentos,resumoVendedores};
}
...
function configurarSistema(){
  ...
  criarAba_(ss,CFG.ABAS.VENDAS,['ID Venda','Data da venda','Data de cadastro','Cliente','CPF/CNPJ','Telefone','ID Produto','Produto','Quantidade','Valor da venda','Custo unitário','Custo total','Lucro','Forma de pagamento','Garantia dias','Observações','Vendedora','Origem do produto','Marca','Modelo','Categoria','Entrada','Saldo a receber','Parcelas JSON','Status recebimento','Recebimentos JSON']);
