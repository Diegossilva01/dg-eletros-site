...
function responderPublico_(e){
  const sh=SpreadsheetApp.getActive().getSheetByName(CFG.ABAS.PRODUTOS);
...
function alterarStatusProduto_(u,d){admin_(u);const sh=SpreadsheetApp.getActive().getSheetByName(CFG.ABAS.PRODUTOS),r=objetos_(sh).find(x=>x.ID===d.id);if(!r)throw new Error('Produto não encontrado.');atualizarLinha_(sh,r._linha,{Status:d.status});return{ok:true}}
function excluirProduto_(u,d){admin_(u);const sh=SpreadsheetApp.getActive().getSheetByName(CFG.ABAS.PRODUTOS),r=objetos_(sh).find(x=>x.ID===d.id);if(!r)throw new Error('Produto não encontrado.');sh.deleteRow(r._linha);log_(u,'Produto',d.id,d.motivo);return{ok:true}}
function cadastrarVenda_(u,d){
  const ss=SpreadsheetApp.getActive(),ps=ss.getSheetByName(CFG.ABAS.PRODUTOS),vs=ss.getSheetByName(CFG.ABAS.VENDAS);
...
function listarUsuarios_(u){admin_(u);const sh=SpreadsheetApp.getActive().getSheetByName(CFG.ABAS.USUARIOS);return{ok:true,usuarios:objetos_(sh).map(x=>({id:x.ID,nome:x.Nome,usuario:x['Usuário'],perfil:x.Perfil,status:x.Status}))}}
function salvarUsuario_(u,d){admin_(u);const sh=SpreadsheetApp.getActive().getSheetByName(CFG.ABAS.USUARIOS),todos=objetos_(sh),r=d.id?todos.find(x=>x.ID===d.id):null;if(todos.some(x=>String(x['Usuário']).toLowerCase()===String(d.usuario).toLowerCase()&&x.ID!==d.id))throw new Error('Esse usuário já existe.');const obj={'ID':r?r.ID:uid_('USR'),'Nome':d.nome,'Usuário':d.usuario,'Senha Hash':d.senha?hash_(d.senha):(r?r['Senha Hash']:''),'Perfil':d.perfil||'Vendedora','Status':d.status||'Ativo','Token':r?r.Token:'','Validade token':r?r['Validade token']:'','Data de cadastro':r?r['Data de cadastro']:new Date()};if(r)atualizarLinha_(sh,r._linha,obj);else appendObj_(sh,obj);return{ok:true}}
...
function migrarProdutosExistentes_(sh){
  ...
  objetos_(sh).forEach(x=>{
    const up={};
    if(!x.ID)up.ID=uid_('PROD');
    if(x.Quantidade===''||x.Quantidade===undefined)up.Quantidade=1;
    if(!x.Status)up.Status='Publicado';
    if(!x.Fotos&&x.Imagem)up.Fotos=JSON.stringify([normalizarImagem_(x.Imagem)]);
    if(!x['Data de cadastro'])up['Data de cadastro']=new Date();
    if(Object.keys(up).length)atualizarLinha_(sh,x._linha,up);
  });
}
