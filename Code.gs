const CFG={ABAS:{PRODUTOS:'Produtos',VENDAS:'Vendas',USUARIOS:'Usuarios',LOG:'LogExclusoes',COMISSOES:'Pagamentos Comissoes'},TOKEN_HORAS:12,PASTA:'DG Eletros - Fotos dos Produtos'};
...
function configurarSistema(){const ss=planilha_();criarAba_(ss,CFG.ABAS.PRODUTOS,['Produto','Categoria','Preço','Preço de custo','Classificação','Descrição','Link da Imagem','Promoção','WhatsApp','ID','Marca','Modelo','Quantidade','Status','Fotos','Data de cadastro']);criarAba_(ss,CFG.ABAS.VENDAS,['ID Venda','Data da venda','Data de cadastro','Cliente','CPF/CNPJ','Telefone','ID Produto','Produto','Quantidade','Valor da venda','Custo unitário','Custo total','Lucro','Forma de pagamento','Garantia dias','Observações','Vendedora','Origem do produto','Marca','Modelo','Categoria','Entrada','Saldo a receber','Parcelas JSON','Status recebimento']);criarAba_(ss,CFG.ABAS.USUARIOS,['ID','Nome','Usuário','Senha Hash','Perfil','Status','Token','Validade token','Data de cadastro']);criarAba_(ss,CFG.ABAS.LOG,['Data','Tipo','ID','Motivo','Usuário']);criarAba_(ss,CFG.ABAS.COMISSOES,['ID','Data do pagamento','Vendedora','Valor pago','Referência','Registrado por']);migrarProdutosExistentes_(ss.getSheetByName(CFG.ABAS.PRODUTOS));const sh=ss.getSheetByName(CFG.ABAS.USUARIOS);if(sh.getLastRow()<2)sh.appendRow([uid_('USR'),'Administrador','admin',hash_('123456'),'Administrador','Ativo','','',new Date()]);return 'Sistema configurado. Login inicial: admin / 123456';}
...
function doPost(e){try{const d=JSON.parse(e.postData.contents||'{}'),a=d.acao;if(a==='login')return json_(login_(d));const u=auth_(d.token);const mapa={verificarToken:()=>({ok:true,usuario:publicUser_(u)}),dashboard:()=>dashboard_(u),listarProdutos:()=>listarProdutos_(u),salvarProduto:()=>salvarProduto_(u,d),alterarStatusProduto:()=>alterarStatusProduto_(u,d),alterarPromocaoProduto:()=>alterarPromocaoProduto_(u,d),excluirProduto:()=>excluirProduto_(u,d),cadastrarVenda:()=>cadastrarVenda_(u,d),listarVendas:()=>listarVendas_(u),listarComissoes:()=>listarComissoes_(u),registrarPagamentoComissao:()=>registrarPagamentoComissao_(u,d),listarContasReceber:()=>listarContasReceber_(u),alterarStatusParcela:()=>alterarStatusParcela_(u,d),baixarParcela:()=>baixarParcela_(u,d),excluirVenda:()=>excluirVenda_(u,d),listarUsuarios:()=>listarUsuarios_(u),salvarUsuario:()=>salvarUsuario_(u,d),excluirUsuario:()=>excluirUsuario_(u,d),listarCatalogoRevendedor:()=>listarCatalogoRevendedor_(u)};if(!mapa[a])throw new Error('Ação inválida.');return json_(mapa[a]())}catch(err){return json_({ok:false,erro:msg_(err)})}}
...
function auth_(token){if(!token)throw new Error('Sessão expirada.');const sh=planilha_().getSheetByName(CFG.ABAS.USUARIOS),u=objetos_(sh).find(x=>String(x.Token||'')===String(token)&&x.Status==='Ativo');if(!u||new Date(u['Validade token']).getTime()<Date.now())throw new Error('Sessão expirada.');const novaValidade=new Date(Date.now()+CFG.TOKEN_HORAS*3600000);atualizarLinha_(sh,u._linha,{'Validade token':novaValidade});u['Validade token']=novaValidade;return u}function admin_(u){if(u.Perfil!=='Administrador')throw new Error('Acesso permitido somente ao administrador.')}function publicUser_(u){return{id:u.ID,nome:u.Nome,usuario:u['Usuário'],perfil:u.Perfil}}
...
function cadastrarVenda_(u,d){
  ...
  const vs=criarAba_(ss,CFG.ABAS.VENDAS,['ID Venda','Data da venda','Data de cadastro','Cliente','CPF/CNPJ','Telefone','ID Produto','Produto','Quantidade','Valor da venda','Custo unitário','Custo total','Lucro','Forma de pagamento','Garantia dias','Observações','Vendedora','Origem do produto','Marca','Modelo','Categoria','Entrada','Saldo a receber','Parcelas JSON','Status recebimento']);
...
function listarVendas_(){
  ...
  const vendas=objetos_(sh).reverse().map(x=>{
    ...
    let parcelas=[];try{parcelas=JSON.parse(x['Parcelas JSON']||'[]')}catch(e){}const totalParcelas=parcelas.length,parcelasPagas=parcelas.filter(p=>String(p.status||'Pendente')==='Pago').length;return{id:x['ID Venda'],data:fmtData_(x['Data da venda']),cliente:x.Cliente,documento:x['CPF/CNPJ'],telefone:x.Telefone,idProduto,produto:x.Produto,quantidade,valor,custoUnitario,custoTotal,lucro,pagamento:x['Forma de pagamento'],garantia:num_(x['Garantia dias']),observacoes:x['Observações'],vendedora:x.Vendedora,origem:x['Origem do produto']||((x['ID Produto'])?'Estoque':'Venda avulsa'),entrada:num_(x.Entrada),saldo:num_(x['Saldo a receber']),parcelas,totalParcelas,parcelasPagas,statusRecebimento:x['Status recebimento']||''};
  });
...
function listarComissoes_(u){
  ...
  const itens=vendas.map(v=>{
    const valor=num_(v.valor),entrada=Math.min(valor,Math.max(0,num_(v.entrada)));
    const parcelas=Array.isArray(v.parcelas)?v.parcelas:[];
    const recebidoParcelas=parcelas.filter(p=>String(p.status||'Pendente')==='Pago').reduce((s,p)=>s+num_(p.valor),0);
    const recebido=String(v.pagamento)==='No prazo'?Math.min(valor,entrada+recebidoParcelas):valor;
    const pendente=Math.max(0,valor-recebido);
    return {...v,recebido,pendente,comissaoGerada:Number((recebido*percentual).toFixed(2)),comissaoPendente:Number((pendente*percentual).toFixed(2)),comissaoTotal:Number((valor*percentual).toFixed(2))};
  });
  ...
  itens.sort((a,b)=>String(a.data||'').localeCompare(String(b.data||''))).forEach(v=>{
    ...
    const disponivel=Math.max(0,num_(v.comissaoGerada));
    const abatido=Math.min(disponivel,Math.max(0,restantePago[chave]||0));
    restantePago[chave]=Math.max(0,(restantePago[chave]||0)-abatido);
    v.comissaoPaga=Number(abatido.toFixed(2));
    v.comissaoLiberada=Number(Math.max(0,disponivel-abatido).toFixed(2));
  });

  const itensAbertos=itens.filter(v=>num_(v.comissaoLiberada)>0.009||num_(v.comissaoPendente)>0.009);
  const totalGerado=itens.reduce((s,v)=>s+num_(v.comissaoGerada),0);
  const totalPendente=itens.reduce((s,v)=>s+num_(v.comissaoPendente),0);
  const totalPago=pagamentos.reduce((s,p)=>s+num_(p.valor),0);
  const saldoPagar=Math.max(0,totalGerado-totalPago);
  const liberadoHoje=itens.filter(v=>isoData_(v.data)===hoje).reduce((s,v)=>s+num_(v.comissaoLiberada),0);
  const liberadoMes=itens.filter(v=>isoData_(v.data).slice(0,7)===mes).reduce((s,v)=>s+num_(v.comissaoLiberada),0);
  const vendedores={};
  itens.forEach(v=>{const nome=v.vendedora||'Sem vendedor';if(!vendedores[nome])vendedores[nome]={vendedora:nome,gerada:0,liberada:0,pendente:0,paga:0};vendedores[nome].gerada+=num_(v.comissaoGerada);vendedores[nome].liberada+=num_(v.comissaoLiberada);vendedores[nome].pendente+=num_(v.comissaoPendente);});
...
function listarContasReceber_(u){
  ...
  if(u.Perfil!=='Administrador')vendas=vendas.filter(v=>String(v.vendedora||'').trim().toLowerCase()===String(u.Nome||'').trim().toLowerCase());
  const hoje=Utilities.formatDate(new Date(),Session.getScriptTimeZone(),'yyyy-MM-dd');
  const limite=new Date(hoje+'T12:00:00');limite.setDate(limite.getDate()+30);const limiteIso=Utilities.formatDate(limite,Session.getScriptTimeZone(),'yyyy-MM-dd');
  const parcelas=[];
  vendas.forEach(v=>{
    let lista=Array.isArray(v.parcelas)?v.parcelas:[];
    // Compatibilidade com vendas antigas que possuem saldo, mas não tinham parcelas gravadas.
    if(!lista.length&&num_(v.saldo)>0)lista=[{numero:1,vencimento:isoData_(v.data)||hoje,valor:num_(v.saldo),status:'Pendente',dataPagamento:''}];
    lista.forEach((p,indice)=>parcelas.push({vendaId:v.id,indice,numero:Number(p.numero||indice+1),totalParcelas:lista.length,vencimento:isoData_(p.vencimento),vencimentoFormatado:fmtData_(p.vencimento),valor:num_(p.valor),status:String(p.status||'Pendente'),dataPagamento:p.dataPagamento?fmtData_(p.dataPagamento):'',cliente:v.cliente,documento:v.documento,telefone:v.telefone,produto:v.produto,valorVenda:num_(v.valor),entrada:num_(v.entrada),saldoVenda:num_(v.saldo),dataVenda:v.data,vendedora:v.vendedora}));
  });
  parcelas.sort((a,b)=>String(a.vencimento||'9999-12-31').localeCompare(String(b.vencimento||'9999-12-31')));
  const pendentes=parcelas.filter(p=>p.status!=='Pago');
  return{ok:true,parcelas,totalReceber:pendentes.reduce((s,p)=>s+num_(p.valor),0),totalVencido:pendentes.filter(p=>p.vencimento&&p.vencimento<hoje).reduce((s,p)=>s+num_(p.valor),0),totalProximos:pendentes.filter(p=>p.vencimento&&p.vencimento>=hoje&&p.vencimento<=limiteIso).reduce((s,p)=>s+num_(p.valor),0),clientesPendentes:new Set(pendentes.map(p=>String(p.vendaId))).size};
}
...
function configurarSistema(){
  ...
  criarAba_(ss,CFG.ABAS.VENDAS,['ID Venda','Data da venda','Data de cadastro','Cliente','CPF/CNPJ','Telefone','ID Produto','Produto','Quantidade','Valor da venda','Custo unitário','Custo total','Lucro','Forma de pagamento','Garantia dias','Observações','Vendedora','Origem do produto','Marca','Modelo','Categoria','Entrada','Saldo a receber','Parcelas JSON','Status recebimento']);
