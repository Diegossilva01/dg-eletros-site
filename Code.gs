...
function configurarSistema(){const ss=planilha_();criarAba_(ss,CFG.ABAS.PRODUTOS,['Produto','Categoria','Preço','Classificação','Descrição','Link da Imagem','Promoção','WhatsApp','ID','Marca','Modelo','Quantidade','Status','Fotos','Data de cadastro']);criarAba_(ss,CFG.ABAS.VENDAS,['ID Venda','Data da venda','Data de cadastro','Cliente','CPF/CNPJ','Telefone','ID Produto','Produto','Quantidade','Valor da venda','Forma de pagamento','Garantia dias','Observações','Vendedora','Origem do produto','Marca','Modelo','Categoria']);criarAba_(ss,CFG.ABAS.USUARIOS,['ID','Nome','Usuário','Senha Hash','Perfil','Status','Token','Validade token','Data de cadastro']);criarAba_(ss,CFG.ABAS.LOG,['Data','Tipo','ID','Motivo','Usuário']);migrarProdutosExistentes_(ss.getSheetByName(CFG.ABAS.PRODUTOS));const sh=ss.getSheetByName(CFG.ABAS.USUARIOS);if(sh.getLastRow()<2)sh.appendRow([uid_('USR'),'Administrador','admin',hash_('123456'),'Administrador','Ativo','','',new Date()]);return 'Sistema configurado. Login inicial: admin / 123456';}
...
function salvarProduto_(u,d){
  ...
  const obj={'Produto':d.nome,'Categoria':d.categoria,'Preço':num_(d.preco),'Classificação':d.classificacao||'A','Descrição':d.descricao||'','Link da Imagem':fotos[0]||'','Promoção':d.promocao!==undefined&&d.promocao!==''?d.promocao:(r?(valorCampo_(r,['Promoção','Promocao'])||'Não'):'Não'),'WhatsApp':r?(valorCampo_(r,['WhatsApp','Whatsapp'])||''):'','ID':id,'Marca':d.marca||'','Modelo':d.modelo||'','Quantidade':num_(d.quantidade),'Status':d.status||'Publicado','Fotos':JSON.stringify(fotos),'Data de cadastro':r?(valorCampo_(r,['Data de cadastro'])||new Date()):new Date()};
  if(r){
  ...
  return{ok:true,id:id,linha:r?r._linha:sh.getLastRow(),preco:num_(d.preco)};
}
...
function cadastrarVenda_(u,d){
  ...
  const vs=criarAba_(ss,CFG.ABAS.VENDAS,['ID Venda','Data da venda','Data de cadastro','Cliente','CPF/CNPJ','Telefone','ID Produto','Produto','Quantidade','Valor da venda','Forma de pagamento','Garantia dias','Observações','Vendedora','Origem do produto','Marca','Modelo','Categoria']);
  ...
  return{ok:true,venda:{id,cliente:d.cliente,documento,telefone,produto,quantidade:qtd,valor,pagamento:d.pagamento,garantia:num_(d.garantia),observacoes:d.observacoes||'',data,vendedora:u.Nome,origem:avulsa?'Venda avulsa':'Estoque'}};
}
...
const CABECALHO_PRODUTOS_FINAL_ = [
  'Produto','Categoria','Preço','Classificação','Descrição','Link da Imagem','Promoção',
...
function configurarSistema(){
  ...
  criarAba_(ss,CFG.ABAS.VENDAS,['ID Venda','Data da venda','Data de cadastro','Cliente','CPF/CNPJ','Telefone','ID Produto','Produto','Quantidade','Valor da venda','Forma de pagamento','Garantia dias','Observações','Vendedora','Origem do produto','Marca','Modelo','Categoria']);
...
function corrigirEstruturaProdutos_(){
  ...
  if(saida.length>1){
    const regraPromocao=SpreadsheetApp.newDataValidation()
      ...
      .build();
    sh.getRange(info.linha+1,7,saida.length-1,1).setDataValidation(regraPromocao);
  }
  sh.setFrozenRows(info.linha);
  SpreadsheetApp.flush();
