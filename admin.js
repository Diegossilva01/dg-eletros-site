...
const state={token:localStorage.getItem('dge_token')||'',user:null,produtos:[],vendas:[],usuarios:[],comissoes:[],receber:[]};
...
async function carregarComissoes(){
  try{
    ...
    tbody.innerHTML=state.comissoes.map(v=>`<tr><td>${esc(v.data)}</td><td>${esc(v.produto)}</td><td>${esc(v.cliente)}</td><td>${money(v.valor)}</td><td>${money(v.recebido)}</td><td>${money(v.pendente)}</td><td><strong>${money(v.comissaoLiberada)}</strong></td><td>${money(v.comissaoPendente)}</td>${isAdmin()?`<td>${esc(v.vendedora)}</td>`:''}</tr>`).join('')||`<tr><td colspan="${isAdmin()?9:8}">Nenhuma comissão em aberto.</td></tr>`;
    const resumo=$('#resumoComissoesBody');
    if(resumo)resumo.innerHTML=(j.resumoVendedores||[]).map(x=>`<tr><td>${esc(x.vendedora)}</td><td>${money(x.liberada)}</td><td>${money(x.pendente)}</td><td>${money(x.paga)}</td><td><strong>${money(x.saldoPagar)}</strong></td><td>${x.saldoPagar>0?`<button type="button" class="table-action action-pay" onclick="pagarComissao('${String(x.vendedora).replace(/'/g,"\\'")}',${Number(x.saldoPagar)||0})">Pagar comissão</button>`:'Sem valor liberado'}</td></tr>`).join('')||'<tr><td colspan="6">Nenhuma comissão pendente.</td></tr>';
...
async function carregarTudo(){if(isRevendedor())return carregarCatalogoRevendedor();await carregarProdutos();await Promise.all([carregarDashboard(),carregarVendas(),carregarReceber(),carregarComissoes(),isAdmin()?carregarUsuarios():Promise.resolve()]);const d=$('#vendaForm [name=dataVenda]');if(d&&!d.value)d.value=hoje()}
...
async function carregarUsuarios(){try{const j=await api('listarUsuarios');state.usuarios=j.usuarios||[];$('#usuariosBody').innerHTML=state.usuarios.map(u=>`<tr><td><div class="employee-name"><span class="employee-avatar">${esc((u.nome||'U').charAt(0).toUpperCase())}</span><strong>${esc(u.nome)}</strong></div></td><td><span class="employee-user">@${esc(u.usuario)}</span></td><td><span class="employee-role">${esc(u.perfil)}</span></td><td><span class="employee-status ${String(u.status).toLowerCase()==='ativo'?'is-active':'is-inactive'}"><i class="fa-solid fa-circle"></i>${esc(u.status)}</span></td><td><div class="employee-actions"><button type="button" class="employee-btn employee-edit" onclick="editarUsuario('${u.id}')"><i class="fa-solid fa-pen-to-square"></i><span>Editar</span></button><button type="button" class="employee-btn employee-delete" onclick="excluirUsuario('${u.id}')"><i class="fa-solid fa-trash-can"></i><span>Excluir</span></button></div></td></tr>`).join('')}catch(e){console.error(e)}}
$('#novoUsuario').onclick=()=>formUsuario();window.editarUsuario=id=>formUsuario(state.usuarios.find(u=>u.id===id));
function formUsuario(u={}){abrirModal(`<h2>${u.id?'Editar':'Novo'} funcionário</h2><form id="usuarioForm" class="grid"><input type="hidden" name="id" value="${esc(u.id||'')}"><label>Nome<input name="nome" required value="${esc(u.nome||'')}"></label><label>Usuário<input name="usuario" required value="${esc(u.usuario||'')}"></label><label>Senha<input name="senha" type="password" ${u.id?'':'required'} placeholder="${u.id?'Deixe em branco para manter':''}"></label><label>Perfil<select name="perfil"><option ${u.perfil==='Vendedora'?'selected':''}>Vendedora</option><option ${u.perfil==='Administrador'?'selected':''}>Administrador</option><option ${u.perfil==='Revendedor'?'selected':''}>Revendedor</option></select></label><label>Status<select name="status"><option ${u.status!=='Inativo'?'selected':''}>Ativo</option><option ${u.status==='Inativo'?'selected':''}>Inativo</option></select></label><div class="full"><button class="primary">Salvar</button></div></form>`);$('#usuarioForm').onsubmit=async e=>{e.preventDefault();try{await api('salvarUsuario',formObj(e.currentTarget));$('#modal').classList.add('hidden');carregarUsuarios()}catch(err){alert(err.message)}}}
iniciarTema();
