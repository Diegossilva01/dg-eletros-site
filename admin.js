...
function formUsuario(u={}){
  abrirModal(`<h2>${u.id?'Editar':'Novo'} funcionário</h2><form id="usuarioForm" class="grid">
    ...
    <div class="full employee-hour-info"><i class="fa-solid fa-calculator"></i><span>Hora normal = salário ÷ (horas semanais × 5). As horas extras são pagas pelo valor normal da hora, sem adicional.</span></div>
...
async function carregarHorasExtras(){
  try{
    ...
    if($('#heValorPendente'))$('#heValorPendente').textContent=money(j.valorPendente||0);
    if($('#heHorasPagas'))$('#heHorasPagas').textContent=formatarMinutosExtras(j.minutosPagos||0);
    if($('#heValorTotal'))$('#heValorTotal').textContent=money(j.valorTotal||0);
    ...
    tbody.innerHTML=state.horasExtras.map(r=>{
      ...
      return `<tr><td><strong>${esc(r.data||'')}</strong></td><td>${esc(r.funcionario||'')}</td><td><strong>${formatarMinutosExtras(r.minutos||0)}</strong></td><td>${esc(r.motivo||'')}</td><td>${money(r.valorHora||0)}</td><td><strong>${money(r.valorTotal||0)}</strong></td><td><span class="overtime-status ${pago?'is-paid':'is-pending'}"><i class="fa-solid fa-circle"></i>${pago?'Pago':'Pendente'}</span></td><td><div class="employee-actions">${acoes.join('')||'—'}</div></td></tr>`;
...
$('#horaExtraForm')?.addEventListener('submit',async e=>{
  ...
  try{const j=await api('registrarHoraExtra',d);f.reset();f.querySelector('[name=data]').value=hoje();f.querySelector('[name=horas]').value=0;f.querySelector('[name=minutos]').value=0;if(msg)msg.textContent=`Registrado: ${formatarMinutosExtras(j.registro?.minutos||0)} • ${money(j.registro?.valorTotal||0)}`;await carregarHorasExtras()}catch(err){if(msg)msg.textContent=err.message}
});
