...
const safeStorage={
  getItem(chave){try{return window.localStorage.getItem(chave)}catch(_){return null}},
  setItem(chave,valor){try{window.localStorage.setItem(chave,valor)}catch(_){}},
  removeItem(chave){try{window.localStorage.removeItem(chave)}catch(_){}}
};
const state={token:safeStorage.getItem('dge_token')||'',user:null,produtos:[],vendas:[],usuarios:[],comissoes:[],receber:[],horasExtras:[]};
...
function protegerSessaoEInterface(){
  ...
  document.addEventListener('click',e=>{
    ...
    if(nav)safeStorage.setItem('dge_pagina_ativa',nav.dataset.page||'dashboard');
  });
  window.addEventListener('beforeunload',()=>{
    if(state.token)safeStorage.setItem('dge_token',state.token);
  });
}
function restaurarPaginaAtiva(){
  const pagina=safeStorage.getItem('dge_pagina_ativa');
...
function aplicarTema(tema){const escuro=tema==='dark';document.body.classList.toggle('dark-theme',escuro);safeStorage.setItem('dge_tema',escuro?'dark':'light');const b=$('#themeToggle');if(b)b.innerHTML=escuro?'<i class="fa-solid fa-sun"></i><span>Tema claro</span>':'<i class="fa-solid fa-moon"></i><span>Tema escuro</span>';}
function iniciarTema(){aplicarTema(safeStorage.getItem('dge_tema')||'light');const b=$('#themeToggle');if(b)b.onclick=()=>aplicarTema(document.body.classList.contains('dark-theme')?'light':'dark');}

// O painel servido pelo Apps Script chama funções do mesmo projeto, sem fetch externo.
const API_TIMEOUT_MS=30000;
function mensagemErroRede(err){return String(err?.message||err||'Falha ao acessar o servidor.')}
function api(acao,dados={}){
  if(typeof google==='undefined'||!google.script?.run){
    return Promise.reject(new Error('Abra o painel pelo link do Google Apps Script.'));
  }
  return new Promise((resolve,reject)=>{
    google.script.run.withSuccessHandler(resposta=>{
      if(resposta?.ok)resolve(resposta);
      else reject(new Error(resposta?.erro||'Falha ao carregar os dados.'));
    }).withFailureHandler(erro=>reject(new Error(mensagemErroRede(erro))))
      .apiInterna({acao,token:state.token,...dados});
  });
}
...
$('#sair').onclick=()=>{safeStorage.removeItem('dge_token');location.reload()};$('#menu').onclick=()=>document.querySelector('aside').classList.toggle('open');
...
function carregarPagina(pagina){const fn=carregamentosPagina[pagina];if(!fn)return Promise.resolve();if(requisicoesPagina[pagina])return requisicoesPagina[pagina];const tarefa=Promise.resolve().then(fn).finally(()=>{delete requisicoesPagina[pagina]});requisicoesPagina[pagina]=tarefa;return tarefa}
async function carregarTudo(){if(isRevendedor())return carregarPagina('revendedor');return carregarPagina($('.nav.active')?.dataset.page||'dashboard')}
...
async function fileData(file){
  ...
  try{
    ...
    const tela=document.createElement('canvas');tela.width=Math.round(bitmap.width*escala);tela.height=Math.round(bitmap.height*escala);
    tela.getContext('2d').drawImage(bitmap,0,0,tela.width,tela.height);bitmap.close();
    return {nome:file.name.replace(/\.[^.]+$/,'.jpg'),data:tela.toDataURL('image/jpeg',.78)};
  }catch(_){return {nome:file.name,data:await ler()}}
}
async function salvarProduto(e){e.preventDefault();const f=e.currentTarget,d=formObj(f);d.preco=parseMoney(d.preco);d.precoCusto=parseMoney(d.precoCusto);d.quantidade=Number(d.quantidade||0);const files=[...f.fotos.files];if(files.length>6)return alert('Selecione no máximo 6 fotos.');d.fotosUpload=await Promise.all(files.map(fileData));const msg=f.querySelector('.msg');msg.textContent='Salvando...';try{await api('salvarProduto',d);$('#modal').classList.add('hidden');await carregarProdutos()}catch(err){msg.textContent=err.message}}
...
window.excluirProduto=async(id,linha)=>{if(!confirm('Excluir definitivamente este produto?'))return;const motivo=prompt('Motivo da exclusão:');if(!motivo)return;try{await api('excluirProduto',{id,linha:Number(linha||0),motivo});await carregarProdutos()}catch(e){alert(e.message)}}
